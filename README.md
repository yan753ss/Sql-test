...
## Чек соответствия заданию

- ✅ Прием имени через GET-параметр — реализовано (`/search-vuln`, `/search`).
- ✅ Уязвимый SQL через конкатенацию — реализовано в `/search-vuln`.
- ✅ Демонстрация SQL-инъекции и UNION-атаки — есть примеры URL.
- ✅ Защита через параметризованные запросы — реализовано в `/search`.
- ✅ Валидация входных данных — regex `^[a-zA-Z0-9]+$` в `/search`.
- ✅ Доп. задание (логирование попыток инъекций) — реализовано middleware.
